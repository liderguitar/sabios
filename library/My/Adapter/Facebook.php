<?php 
class My_Adapter_Facebook implements Zend_Auth_Adapter_Interface
{
    /**
    ...
    public function authenticate()
    {
    	...
    	if ( empty ($code ) )
    	{
	    	...
	    	$loginUri = sprintf(My_Adapter_Facebook::AUTH_URI , $this->_appId, $this->_redirectUri);
    	...
    	else
    	{
	    	...
	    	$client = new Zend_Http_Client( My_Adapter_Facebook::TOKEN_URI );
	    	...
	    	$client = new Zend_Http_Client(My_Adapter_Facebook::USER_URI );
	    	$client->setParameterGet('client_id', $this->_appId);
	    	$client->setParameterGet('access_token', $params['access_token']);
                
	    	$user = json_decode($result->getBody());
	    	$usuario = Usuario::verifyExternalUser('facebook', $user->id, $user->name, $user->firstName, $user->lastName);
			
            return new Zend_Auth_Result(  Zend_Auth_Result::SUCCESS , $usuario, array( 'token'=>$params['access_token']) );
    	}
